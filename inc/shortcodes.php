...
<?php while($wp_query->have_posts()): $wp_query->the_post();
		...
		echo '<div class="vtitle"><a class="text-dark secondary-font fw-bold" href="'.get_the_permalink().'">'.substr($vtitle, 0, 60) . ' ...'.'</a></div>';
...
function velocity_popular_posts($atts){
	ob_start();
  $atribut = shortcode_atts( array(
      'showposts'   => 10,
  ), $atts );
	...
	$args['order'] = 'DESC';
	$args['showposts'] = $atribut['showposts'];	;
	$wpex_query = new wp_query( $args );
	if($wpex_query->have_posts()):
	echo '<div class="velocity-popular-posts">';
		while($wpex_query->have_posts()): $wpex_query->the_post(); ?>
		<div class="velocity-popular-list mb-3">
			<div class="fw-bold mb-0"><a class="text-dark" href="<?php echo get_the_permalink($post->ID); ?>"><b><?php echo get_the_title($post->ID); ?></b></a></div>
			<small class="text-secondary"><?php velocity_post_date(get_the_ID());?>  |  <?php echo get_post_meta(get_the_ID(), 'hit', true);?> dilihat</small>
		</div>
	<?php endwhile;
	echo '</div>';
	endif;
...
function velocity_recent_posts($atts){
    ...
    if($style == 'first_image' && $no == '1'){
      ...
      <div class="col-content<?php echo $col2;?>">
        <div class="v-post-title fw-bold mb-0"><a class="text-dark" href="<?php echo get_the_permalink($post->ID); ?>"><b><?php echo get_the_title($post->ID); ?></b></a></div>
        <small class="v-post-date text-secondary fst-italic"><?php velocity_post_date(get_the_ID());?></small>
		  </div>
		</div>
	<?php endwhile;
	echo '</div>';
