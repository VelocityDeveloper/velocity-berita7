...
function velocitychild_theme_setup() {	
	...
	if (class_exists('Kirki')) :
		...
		Kirki::add_panel('panel_berita', [
			'priority'    => 10,
			'title'       => esc_html__('Velocity Berita', 'justg'),
		...
		$fieldiklan = [
			'iklan_header'  => [
				'label'			=> 'Iklan Header',
				'description'	=> 'Ukuran gambar 955x90',
			],
			'iklan_single'  => [
...
// Unregister sidebars
function velocity_remove_widgets(){
...
add_action( 'widgets_init', 'velocity_remove_widgets', 11 );
...
function velocity_posts($cat_id = null) {
    ...
    if(!empty($velocity_posts)){
		echo '<div class="bg-dark text-white px-3 py-2 fs-6 mb-2">'.$category_name.'</div>';
        echo '<div class="row mb-4">';
            echo '<div class="col-md-6 text-muted">';
            foreach(array_slice($velocity_posts, 0,1) as $post) {
                $post_id = $post->ID;
                echo '<div class="h-100 bg-dark text-white">';
                echo '<div class="position-relative">';
                	echo do_shortcode('[resize-thumbnail width="410" height="280" linked="true" class="w-100" post_id="'.$post_id.'"]');
					echo '<small class="position-absolute bottom-0 start-0 py-2 px-3">';
                ...
                echo '<div class="p-3">';
					...
					echo do_shortcode('[velocity-excerpt count="70" post_id="'.$post_id.'"]');
				echo '</div>';
				echo '</div>';
            }
            echo '</div>';
            echo '<div class="col-md-6 ps-md-0">';
                echo '<div class="row mx-0 background-list">';
                foreach(array_slice($velocity_posts, 1,4) as $post) { 
					$post_id = $post->ID;                       
                    echo '<div class="col-6 col-md-12 px-0 li">';
					echo '<div class="row me-sm-0">';
                        echo '<div class="velocity-post-thumbnail col-sm-4 col-md-5 pe-sm-0 mb-2 mb-sm-0">';
                            echo do_shortcode('[resize-thumbnail width="250" height="180" linked="true" class="w-100" post_id="'.$post_id.'"]');
                        echo '</div>';
                        echo '<div class="velocity-post-title col-sm-8 col-md-7">';
							echo '<div class="pt-1 px-3 px-sm-0 pb-sm-0 pb-3">';
								echo '<a class="judul-3-baris text-white d-block w-100" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</a>';
							echo '</div>';
                        echo '</div>';
                    echo '</div>';
                    echo '</div>';
...
function velocity_posts2($cat_id = null) {
    $args['post_type'] = 'post';
    $args['showposts'] = 6;
    if (!empty($cat_id)){
        $args['cat'] = $cat_id;
    ...
    if(!empty($velocity_posts)){
        ...
        echo '<div class="row mb-4">';
            ...
            foreach(array_slice($velocity_posts, 0,1) as $post) {
                $post_id = $post->ID;
                echo '<div class="h-100 text-white">';
                echo '<div class="position-relative">';
                	echo do_shortcode('[resize-thumbnail width="410" height="350" linked="true" class="w-100" post_id="'.$post_id.'"]');
					echo '<div class="position-absolute bottom-0 start-0 pb-2 px-3">';
						echo '<a class="text-shadow fw-bold text-white lh-sm d-block" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</a>';
					echo '</div>';
				echo '</div>';				
				echo '</div>';
            }
            echo '</div>';
            echo '<div class="col-md-6 ps-md-0">';
                echo '<div class="h-100 bg-success">';
                foreach(array_slice($velocity_posts, 1,4) as $post) {
					$post_id = $post->ID;
                    echo '<div class="py-2 px-3 border-bottom">';
						echo '<a class="judul-2-baris text-white d-block w-100" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</a>';
                    echo '</div>';
                }
                echo '</div>';
            echo '</div>';
...
function velocity_post_loop($post_id = null) {
	...
	echo '<div class="row">';
		...
		echo '<div class="col-right col-8 ps-md-4">';
			echo '<div class="fs-title mb-2 lh-sm"><a class="fw-bold" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</a></div>';
			echo '<div class="mb-3">';
				echo do_shortcode('[velocity-excerpt count="150" post_id="'.$post_id.'"]');
			echo '</div>';
		    echo '<div class="px-3 py-2 bg-color-theme text-white">';
				echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="me-1 bi bi-person-fill" viewBox="0 0 16 16"><path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6"/></svg>';
				echo '<small>'.$author_name.'</small>';
				echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="ms-3 me-2 bi bi-calendar-check" viewBox="0 0 16 16"><path d="M10.854 7.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 9.793l2.646-2.647a.5.5 0 0 1 .708 0"/><path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z"/></svg>';
				echo '<small>';
					velocity_post_date($post_id);
				echo '</small>';
			echo '</div>';
